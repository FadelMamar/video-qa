...
class Signature(dspy.Signature):
    ...
    images: List[dspy.Image] = dspy.InputField(desc="image from videosurveillance camera")
    analysis: str =  dspy.OutputField(desc="description of images' content and human activity taking place")
    

def load_dspy_model(model:str,temperature:float,model_type:str='chat',cache:bool=True):
...
class Analyzer(ABC):
    ...
    def format_user_message_images(self,
    ...
    # TODO: debug      
    def format_user_message_video(self,
                            images:Sequence[bytes],
    ...
    def inference_with_api(self,images:list[bytes],
                           prompt:str="Describe clearly what you see in the video",
                           sys_prompt:str=None,
        ...
        if as_video:
            user_messages = self.format_user_message_video(images=images,
                                                           prompt=prompt)
        else:
            user_messages = self.format_user_message_images(images=images,
                                                           prompt=prompt)
...
class DspyAnalyzer(Analyzer):
    ...
    def __init__(self,
        ...
        lm = load_dspy_model(model=model,
                             temperature=temperature,
                             model_type='chat',
                             cache=cache)        
...
class Summarizer:
    ...
    def __init__(self,model:str='openai/Qwen2.5-VL-3B',
                 cache:bool=True,
                 temperature:float=0.5,
                 ):
        
        self.lm = load_dspy_model(model=model,
                             temperature=temperature,
                             model_type='text',
                             cache=cache)
